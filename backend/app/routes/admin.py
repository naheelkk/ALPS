...
def create_lesson(course_id):
    ...
    topics_list = [t.strip() for t in topic_str.split(',')] if topic_str else []
...
def update_lesson(lesson_id):
    ...
    if 'topic' in data:
        topic_str = data['topic']
        topics_list = [t.strip() for t in topic_str.split(',')] if topic_str else []
        lesson.topics = json.dumps(topics_list)
...
@admin_bp.route('/lessons/suggest-topic', methods=['POST'])
@tutor_required
def suggest_lesson_topic_route():
    data = request.get_json()
    ...
    content_blocks = data.get('contents', [])

    from app.services.topic_tagger import suggest_lesson_topic
    result = suggest_lesson_topic(lesson_title, lesson_description, content_blocks, course_title)

    if result.get('error'):
        # Just return the error message without 500 so UI can handle gracefully if API key is missing
        return jsonify({'message': f"LLM error: {result['error']}", **result}), 200
...
def create_question(quiz_id):
    ...
    # Sync quiz topics
    if question.concept and question.concept != 'General':
        existing_topics = json.loads(quiz.topics) if quiz.topics else []
        if question.concept not in existing_topics:
            existing_topics.append(question.concept)
            quiz.topics = json.dumps(existing_topics)
...
def suggest_question_topic():
    ...
    if result.get('error'):
        # Just return the error message without 500 so UI can handle gracefully
        return jsonify({'message': f"LLM error: {result['error']}", **result}), 200
...
@admin_bp.route('/courses/<int:course_id>/analytics', methods=['GET'])
@tutor_required
def get_course_analytics(course_id):
    ...
    # Total Views & Unique Visitors (Approximated with Enrollments)
    enrollments = Enrollment.query.filter_by(course_id=course_id).all()
    unique_visitors = len(enrollments)
    views = unique_visitors * 3 # Simple mock multiplier for views
    ...
    # Calculate Revenue
    revenue = len(enrollments) * course.price

    # Get Quiz Scores for Distribution
    submissions = Submission.query.join(Quiz).filter(Quiz.course_id == course_id).all()
    quiz_scores = [
        {'range': '0-20%', 'count': sum(1 for s in submissions if s.score <= 20)},
        {'range': '21-40%', 'count': sum(1 for s in submissions if 20 < s.score <= 40)},
        {'range': '41-60%', 'count': sum(1 for s in submissions if 40 < s.score <= 60)},
        {'range': '61-80%', 'count': sum(1 for s in submissions if 60 < s.score <= 80)},
        {'range': '81-100%', 'count': sum(1 for s in submissions if s.score > 80)}
    ]
    
    # Mock Enrollment Trend (Last 12 Weeks)
    ...
    for i in range(12, 0, -1):
        ...
        enrollment_trend.append(count)

    return jsonify({
        'views': views,
        'unique_visitors': unique_visitors,
        'avg_time': '2h 30m', # Hardcoded for now
