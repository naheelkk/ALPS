...
**`Lesson`** — Individual lessons within a course, ordered by `order`. Contains a collection of `LessonContent` blocks, allowing rich mixed-media lessons. `is_free` flag allows preview without enrollment. Also contains a `topics` JSON array used to define its feature vector for the CMAB engine.

**`LessonContent`** — A single content block within a lesson. Types include: `video`, `text`, `code`, `image`, or `file`. Stores the specific content type data (e.g., `body` and `language` for code, `url` and `file_name` for files).
...
#### `BanditParam` (`app/models/bandit.py`)

Stores the learned weights for the Contextual Multi-Armed Bandit (CMAB) algorithm. Because the system can recommend both internal `Lesson`s and external `Resource`s, this table uses a **polymorphic design**:

| Field | Description |
|---|---|
| `item_type` | `'lesson'` or `'resource'` |
| `item_id` | Foreign Key ID to the respective table |
| `matrix_a` | The $d \times d$ inverse covariance matrix $A$, stored as a flattened JSON array |
| `vector_b` | The $d$-dimensional reward vector $b$, stored as a JSON array |
...
- **Lessons** — Add/edit/delete/reorder lessons. Features a rich `LessonContentPanel` allowing administrators to build lessons using multiple sequential content blocks (text, video, code snippets, images, and file attachments) within a single lesson.
...
| Rich multi-block lesson content (text, video, code, images, files) | ✅ Done |
...
| Dark Mode support & UI themes | ✅ Done |
